...
void sys_practice(struct intr_frame* f);
...
void sys_compute_e (struct intr_frame *f);
...
syscall_init (void) 
{
  ...
  //implementation for Task2: initialize halt,exit,exec
  syscalls[SYS_EXEC] = &sys_exec;
  syscalls[SYS_HALT] = &sys_halt;
  syscalls[SYS_EXIT] = &sys_exit;

  //add extra tests [96 tests version]
  syscalls[SYS_PRACTICE]=&sys_practice;
  syscalls[SYS_COMPUTE_E] = &sys_compute_e;
 
  //implementation for Task3: initialize create, remove, open, filesize, read, write, seek, tell, and close
